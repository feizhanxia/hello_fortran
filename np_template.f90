...
! ccccccccccccccccccrccccccccccccccccccccccccccccccccccccccccc
        module systems
! c      两体系统的变量
...
! ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        ...
        do while (abs(delta)>1e-6) !寻找合适的势阱深度
            n=n*(1+delta)
...
! cccccccccccccccccccccccccc请完善Numerov方法ccccccccccccccccccccc
        ! 1.通过phi(x+h)的递推式即Numerov算法得到phi(irmid)
        !!!!!!!!
        ! k^2=((-2*mu/(hbarc**2))*(vpot(ir)-ecm))
            do ir=2, irmid+3
            phi(ir)=(2*(1-5*(hcm**2)*((-2*mu/(hbarc**2))*(vpot(ir-1)-ecm))/12)*phi(ir-1) &
            -(1+(hcm**2)*((-2*mu/(hbarc**2))*(vpot(ir-2)-ecm))/12)*phi(ir-2)) &
            /(1+(hcm**2)*((-2*mu/(hbarc**2))*(vpot(ir)-ecm))/12)
            end do 
        ...
        ! 2.通过phi(x+h)的递推式即Numerov算法得到phi(irmid)
            do ir=irmatch-2, irmid-3, -1
            phi(ir)=(2*(1-5*(hcm**2)*((-2*mu/(hbarc**2))*(vpot(ir+1)-ecm))/12)*phi(ir+1) &
            -(1+(hcm**2)*((-2*mu/(hbarc**2))*(vpot(ir+2)-ecm))/12)*phi(ir+2)) &
            /(1+(hcm**2)*((-2*mu/(hbarc**2))*(vpot(ir)-ecm))/12)
            end do 

! cccccccccccccccccccccccccc请完善Numerov方法cccccccccccccccccccccc
